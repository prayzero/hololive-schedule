...
- `public/data/youtube-lives.json`
  ...
  - JP 자동 수집은 공식 채널 ID와 라이브 메타데이터를 재검증합니다.
    검색 구조·메타데이터 요청 실패나 전체 0건은 정상 갱신으로 기록하지 않습니다.
  - YouTube가 검색 데이터를 제공하지 않으면 기존 검증 자료와 확인 시각을
    유지하고 Actions에 경고를 남깁니다. 이 경우 방송 일정 배포는 계속하며,
    보안·데이터 검증 오류는 여전히 배포를 중단합니다.
- `public/data/music.json`
  ...
  - 원본 표의 열 이름 대소문자를 정규화하고 필수 열·분류별 수집량을 검사합니다.
    기존 수록곡은 원본 표에서 빠졌다는 이유만으로 삭제하지 않습니다.
- `public/data/hololive-dreams.json`
  - `hololive Dreams` 공식 사이트의 출시 참여 멤버 54명과 공식 캐릭터
    썸네일을 담습니다.
  - 공식 X와 개발사가 게시한 App Store 이벤트에서 확인한 행사·픽업 일정을
    담습니다. 챕터·카드가 미공개인 행사는 공식 배너와 참여 멤버만 표시합니다.
    공식 근거 없는 종료 시각과 신규 픽업 확률은 추정해서 채우지 않습니다.
...
- `public/data/hololive-wafers.json`
  ...
  - 카드게임용 트윈 웨하스는 hOCG 탭으로 분리합니다. 전체 카드 이미지가
    미공개인 시리즈도 공식 발표된 발매 월·종류 수는 별도 예고로 표시합니다.
...
- 외부 읽기 요청의 일시적인 연결·서버 오류는 최대 2회 재시도합니다. 권한 오류,
  허용되지 않은 주소, 응답 크기 초과와 데이터 검증 실패는 재시도로 숨기지 않습니다.
- 예약 Actions는 GitHub 사정에 따라 설정한 15분 간격보다 늦어질 수 있습니다.
